...
void maketwotone(double freq, double freq2, unsigned char *wave, int samples)
{
	int	i;
	double inc1;
	double pos1;
	...
	inc1=8.0*atan(1.0);
	inc1 *= ((double)freq / (double)S_RATE);
	inc2=8.0*atan(1.0);
	inc2 *= ((double)freq2 / (double)S_RATE);

	for(i=0;i<samples;i++) {
		pos1=(inc1*(double)i);
		pos1 -= (int)(pos1/WAVE_TPI)*WAVE_TPI;
		pos2=(inc2*(double)i);
		pos2 -= (int)(pos2/WAVE_TPI)*WAVE_TPI;
		wave[i]=(sin (pos1))*63+64;
		wave[i]+=((sin (pos2))*63+64);
	}
}
...
{
	unsigned char	wave[S_RATE*30];
	int samples;

	maketwotone(1000, 1575, wave, S_RATE*30);
	maketwotone(1000, 1000, wave, S_RATE*30);
	printf("%d\n",xp_play_sample(wave, S_RATE*30, FALSE));
	return(0);
}
